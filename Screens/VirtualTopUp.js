...
import { SupplierBillerCommission, SupplierSelf, SupplierThirdParty, SupplierUrl, SupplierVtuAirtime, SupplierVtuData, ValidatePin } from '../Utils/AuthRoute';
...
export const VirtualTopUp = ({navigation, route}) => {
    ...
    useEffect(() => {
      const unsubscribe = navigation.addListener('focus', async () => {
        try {
          ...
          setpincheckifempty(response.data.data.transaction_pin_setup)
          setisloading(false)
        } catch (error) {
          setisloading(true)
...
const getBouquets = (value) => {
  ...
  const TopupAirtime = () => {
    ...
    const mobilevalidationThirdparty = async () => {
      try {
        setisloading(true)
          const response = await SupplierThirdParty(authCtx.Id, phoneValidation, authCtx.token)
    ...
    const mobilevalidationSelf = async () => {
      try {
          setisloading(true)
          const response = await SupplierSelf(authCtx.Id, authCtx.token)
          // console.log(response)
          setRequestId(response.requestID)
          if(response.status === "Success"){
              if(DATACHECK){
  ...
  const commissionget = async (id) => {
     try {
       const response = await SupplierBillerCommission(id, authCtx.token)
       console.log(response)
  ...
  const airtimetopup = async () => {
    ...
    try {
      setisloading(true)
        const response = await SupplierVtuAirtime(requestId, id, amount, authCtx.token, commissonvalue)
        // console.log(response)
...
const datatoptup = async() => {
    ...
    try {
      setisloading(true)
        const response = await SupplierVtuData(requestId, id, bosquetPrice, bosquetData, authCtx.token, commissonvalue)
        // console.log(response)
